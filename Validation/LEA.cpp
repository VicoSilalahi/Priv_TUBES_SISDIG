...
WORD reverseBits(WORD n) {
    WORD reversed = 0;
    int bitCount = sizeof(WORD) * 8; // Get the number of bits in WORD (usually 32 or 64)

    for (int i = 0; i < bitCount; ++i) {
        if (n & (1UL << i)) { // Check if the i-th bit is set
            reversed |= (1UL << (bitCount - 1 - i)); // Set the corresponding bit in reversed
        }
    }
    return reversed;
}

std::string toBinaryString(WORD n) {
    int bitCount = sizeof(WORD) * 8; // Get the number of bits in WORD
    std::string binary;
...
void KeySchedule_128(BYTE *K, WORD *RK)
{
	...
	for (int i = 0; i < 24; i++)
	{
		//std::cout << std::hex << std::setw(8) << std::setfill('0')<<(unsigned int)T[0]<<" ";
		//std::cout << std::hex << std::setw(8) << std::setfill('0')<<(unsigned int)T[1]<<" ";
		//std::cout << std::hex << std::setw(8) << std::setfill('0')<<(unsigned int)T[2]<<" ";
		//std::cout << std::hex << std::setw(8) << std::setfill('0')<<(unsigned int)T[3]<<" ";
		...
		// std::cout << std::hex << std::setw(8) << std::setfill('0')<<toBinaryString(RK[i * 6 + 0])<<" ";
		// std::cout << std::hex << std::setw(8) << std::setfill('0')<<(unsigned int)RK[i * 6 + 1]<<" ";
		// std::cout << std::hex << std::setw(8) << std::setfill('0')<<(unsigned int)RK[i * 6 + 2]<<" ";
		// std::cout << std::hex << std::setw(8) << std::setfill('0')<<(unsigned int)RK[i * 6 + 3]<<" ";
		// std::cout << std::hex << std::setw(8) << std::setfill('0')<<(unsigned int)RK[i * 6 + 4]<<" ";
		// std::cout << std::hex << std::setw(8) << std::setfill('0')<<(unsigned int)RK[i * 6 + 5]<<" ";
...
void Encrypt(int Nr, WORD *RK, BYTE *P, BYTE *C)
{
	...
	for (int i = 0; i < Nr; i++)
	{
		//std::cout<<std::hex<<std::setw(8) << std::setfill('0')<<X_Round[3]<<" ";
		//std::cout<<std::hex<<std::setw(8) << std::setfill('0')<<X_Round[2]<<" ";
		//std::cout<<std::hex<<std::setw(8) << std::setfill('0')<<X_Round[1]<<" ";
		//std::cout<<std::hex<<std::setw(8) << std::setfill('0')<<X_Round[0]<<" ";
